...
test('multiline text input insertion preserves newlines', () => {
  const inserted = insertTextInputText('firstsecond', 5, '\n')

  assert.equal(inserted.value, 'first\nsecond')
  assert.equal(inserted.cursor, 6)
})

test('multiline text input treats shift-enter and alternate enter as soft breaks', () => {
  assert.equal(isTextInputSoftBreak({ return: true, shift: true }), true)
  assert.equal(isTextInputSoftBreak({ return: true, meta: true }), true)
  assert.equal(isTextInputSoftBreak({ return: true }), false)
})

...
test('select starts on the first selectable item when a utility back option is present', () => {
  const output = renderToString(
    React.createElement(AppInputProvider, null,
      React.createElement(Select<'simple' | 'advanced' | 'back'>, {
        options: [
          { value: 'simple', label: 'Simple', hint: 'Connected wallet owns token and agent ENS subdomain' },
          { value: 'advanced', label: 'Advanced', hint: 'Owner wallet owns token and agent ENS subdomain' },
  ...
  assert.doesNotMatch(output, /> Back/)
})

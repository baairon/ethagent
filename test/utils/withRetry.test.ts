...
test('fetchWithRetry treats parseRetryHintFromBody.fatal as non-retryable on 429', async () => {
  let calls = 0
  const sleeps: number[] = []
  const onRetry: unknown[] = []
  const response = await fetchWithRetry('https://example.invalid/endpoint', { method: 'POST' }, {
    maxRetries: 4,
    sleep: async ms => { sleeps.push(ms) },
    fetchImpl: (async () => {
      ...
      return new Response(JSON.stringify({
        error: {
          ...
          details: [
            { '@type': 'type.googleapis.com/google.rpc.QuotaFailure', violations: [{ metric: 'foo', quotaValue: '20' }] },
            { '@type': 'type.googleapis.com/google.rpc.RetryInfo', retryDelay: '22.7s' },
          ],
        },
      }), { status: 429, headers: { 'content-type': 'application/json' } })
    }) as typeof fetch,
    onRetry: e => onRetry.push(e),
    parseRetryHintFromBody: (text) => {
      const body = JSON.parse(text) as { error?: { details?: Array<{ '@type'?: string }> } }
      const isQuota = !!body.error?.details?.some(d => /QuotaFailure$/.test(d['@type'] ?? ''))
      return isQuota ? { fatal: true, reason: 'quota exhausted' } : undefined
    },
  })

  assert.equal(response.status, 429)
  assert.equal(calls, 1, 'should not retry on fatal hint')
  assert.deepEqual(sleeps, [], 'should not sleep at all')
  assert.deepEqual(onRetry, [], 'should not emit retry events')
})

test('fetchWithRetry honors parseRetryHintFromBody.retryAfterMs when no header is present', async () => {
  let calls = 0
  const sleeps: number[] = []
  const response = await fetchWithRetry('https://example.invalid/endpoint', { method: 'POST' }, {
    maxRetries: 1,
    ...
    fetchImpl: (async () => {
      ...
      if (calls === 1) {
        return new Response(JSON.stringify({
          error: { details: [{ '@type': '.../RetryInfo', retryDelay: '1.5s' }] },
        }), { status: 429, headers: { 'content-type': 'application/json' } })
      }
      return new Response('ok', { status: 200 })
    }) as typeof fetch,
    parseRetryHintFromBody: () => ({ retryAfterMs: 1500, reason: 'rate limit' }),
  })
...
test('fetchWithRetry skips parseRetryHintFromBody for non-429 retryable statuses', async () => {
  let parserCalls = 0
  let calls = 0
  await fetchWithRetry('https://example.invalid/endpoint', { method: 'POST' }, {
    maxRetries: 1,
    ...
    parseRetryHintFromBody: () => { parserCalls += 1; return undefined },
  })

  assert.equal(parserCalls, 0, 'parser should only run for 429s')
})

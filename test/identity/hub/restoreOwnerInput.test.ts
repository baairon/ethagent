...
test('restore flow starts with wallet connection and backs to hub', () => {
  const fromMenu = identityHubReducer({ kind: 'menu' }, { type: 'setStep', step: { kind: 'restore-wallet' } })
  assert.deepEqual(fromMenu, { kind: 'restore-wallet' })

  const walletBack = identityHubReducer({ kind: 'restore-wallet', purpose: 'switch' }, {
    type: 'back',

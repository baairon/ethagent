...
} from '../../../../src/identity/manager/shared/model/copy.js'
...
test('identity manager formats blocked registration without raw wallet language', () => {
  const view = identityManagerErrorView(new RegisterAgentPreflightError({
...
test('identity manager capitalizes generic identity errors', () => {
...
test('identity manager explains locked backups without generic decrypt copy', () => {
  const view = identityManagerErrorView(new AgentStateOwnerMismatchError(
...
test('identity manager explains missing restore access without generic identity error copy', () => {
  const view = identityManagerErrorView(new Error('Restore Slot Missing: re-save this agent once with the owner wallet.'))
...
test('identity manager local change status distinguishes clean and changed files', () => {
  const published = {
    'SOUL.md': 'soul-a',
    'MEMORY.md': 'memory-a',
...
test('copyableIdentityFields exposes advanced ENS operator wallet state', () => {
  ...
  assert.deepEqual(fields.map(field => field.label), ['Token ID', 'ENS Name', 'Agent URI', 'Owner Wallet', 'Vault', 'Operator Wallet'])
...
test('setup and wallet copy uses owner/operator wording in user-facing labels', () => {
  ...
  const ownerError = identityManagerErrorView(new Error('owner wallet required: this needs the owner wallet'))
  assert.equal(ownerError.title, 'Owner Wallet Required')

  const operatorError = identityManagerErrorView(new Error('operator wallet required: sign with operator wallet'))

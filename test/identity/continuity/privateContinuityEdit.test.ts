...
import { permissionOptionsForRequest } from '../../../src/chat/views/PermissionPrompt.js'
import {
  continuityVaultRef,
  readContinuityFiles,
...
test('approved private continuity edit records identity history but no rewind checkpoint', async () => {
  await withHome(async () => {
    ...
    assert.match(outcome.result.content, /Identity Hub > Soul, Memory, and Skills/)
    assert.match(outcome.result.content, /Identity Hub > Recovery > Save Snapshot Now/)

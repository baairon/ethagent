...
test('README documents core-file sync, custody modes, ENS naming, and transfer', () => {
  ...
  assert.match(readme, /With the Claude Code plugin this is automatic/)
  assert.match(readme, /npx ethagent --sync/)
  assert.match(readme, /--sync-list/)
  assert.match(readme, /lifecycle hooks/)
  assert.match(readme, /Sync is not backup/)
  assert.match(readme, /Custody/)
  assert.match(readme, /\*\*Simple\.\*\*/)
  assert.match(readme, /An owner wallet holds the agent in a Vault/)
  assert.match(readme, /operators can \*\*never\*\* move or sell your agent/)
  assert.match(readme, /under an ENS name you own/)
  assert.match(readme, /found by ENS name or token id/)

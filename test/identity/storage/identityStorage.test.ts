...
test('setTokenIdentity backfills config.erc8004 from identity chain info when absent', async () => {
  await withTempHome(async () => {
    const config = baseConfig()
    await saveConfig(config)
    const identity = {
    ...
    const updated = await setTokenIdentity(config, identity)
    assert.equal(updated.erc8004?.chainId, 8453)
    assert.equal(updated.erc8004?.identityRegistryAddress, identity.identityRegistryAddress)
    assert.equal(updated.erc8004?.rpcUrl, identity.rpcUrl)
  })
})

test('setTokenIdentity preserves an existing erc8004 block, including operatorVaults', async () => {
  await withTempHome(async () => {

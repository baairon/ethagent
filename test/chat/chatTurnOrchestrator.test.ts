...
test('runStreamingTurn executes direct cd requests without asking the model', async () => {
  ...
  assert.ok(rows.some(row => row.role === 'tool_call' && row.name === 'change_directory' && row.result))
})
...
test('runStreamingTurn silences provider retry chatter while preserving streamed output', async () => {
  const cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'ethagent-orch-'))
  const sessionMessages: SessionMessage[] = []
  const rows: MessageRow[] = []
  ...
  assert.equal(result.finishedNormally, true)
  assert.ok(!notes.some(note => /provider retry/i.test(note.text)),
    'retry events should not surface as user-visible notes')
...
test('runStreamingTurn stops when model emits no tool_uses (model decides it is done)', async () => {
  ...
  assert.ok(rows.every(row => row.role !== 'tool_call' || row.name !== 'tool_requirement'))
})
...
test('runStreamingTurn feeds tool errors back to model as tool_results', async () => {
  ...
  assert.equal(result.finishedNormally, true)
  assert.equal(providerCalls, 2)
  assert.ok(rows.some(row => row.role === 'tool_call' && row.name === 'edit_file' && row.result?.isError))
